...
- `meta/` — session notes (from `/save`) and lint reports (from `/lint`). Latest: [[meta/lint-report-2026-07-02]] (prior: [[meta/lint-report-2026-06-26]]).
  - [[gunnerteam/meta/session-2026-07-02-cc3107-weather-smoke-test]] — 2026-07-02: cc-3107 — `gunnerteam-api/scripts/smoke-weather.mjs`, a repeatable prod-safe self-cleaning smoke test for the danger-only weather pipeline (cc-3101 engine / cc-3103 routes), run against live dev via the `_sql` seam + `weather-sweep` task dispatch. **10/10 live.** Fixed two real bugs in the adopted script: (1) FORCE-RLS blindness — plain `gt_weather_alerts` ops through the `_sql` seam silently return zero rows / are policy-denied, needs MATERIALIZED-CTE `set_config` for reads+insert-returning (single-statement, since multi-statement `SET; SELECT` loses rows) and SET-prefix for row-less UPDATEs; (2) dedup assertion filtered `ended_at IS NULL` but the unique index excludes `ended_at` → flakes vs the 15-min cron close-out, retargeted to "exactly one row ever." Also: Cognito client has `USER_PASSWORD_AUTH` not `ADMIN_USER_PASSWORD_AUTH`; `_sql`/task dispatch both work via `--qualifier live`. Committed `bf2373f`.
  - [[meta/session-2026-07-02-gunner-prefix-scripted-pass]] — 2026-07-02: **executed** the scripted pass — 298 substitutions / 111 files, dry-run matched plan, diffs spot-checked, grep-verified zero remaining `[[gunner/...]]` wikilinks outside intentional exclusions.
  - [[meta/scripted-pass-plan-gunner-prefix-links-2026-07-02]] — 2026-07-02: plan (executed same day, see session note above) for a dedicated scripted pass retiring 298 stale `gunner/` prefix wikilinks (31 verified canonical-target mappings, dry-run script included). Separate from the same-day lint's 37-link `gunner/system-security-plan` ambiguous-collision fix.
  - [[gunnerteam/meta/session-2026-07-02-cc3506-docusign-co-confirm-guard]] — 2026-07-02: cc-3506 — incident-driven guard: `.confirmationDialog` on `PDFChangeOrderView` submit button shows real homeowner email + CO amount before firing DocuSign; BUILD SUCCEEDED; TestFlight 3.3.4 (17) uploaded (includes cc-3502..3506).
  - [[gunnerteam/meta/session-2026-07-02-cc3500-3501-totp-mfa-login-settings]] — 2026-07-02: TOTP MFA for GunnerTeam iOS — cc-3500 login challenge handling (`AuthManager.login()` switches on `nextStep`, `confirmSignInWithTOTPCode` path, `MFAEnrollmentView` blocking full-screen QR setup) + mandatory enrollment for admins (`ContentView` root-gated fullScreenCover); cc-3501 optional opt-in/out for all other roles via a new Settings "Security" section (shared `MFAEnrollmentView(mode: .optional)`, disable path guarded against admins). Key finding: `fetchMFAPreference`/`updateMFAPreference` are `AWSCognitoAuthPlugin`-specific, not on `Amplify.Auth` directly — verified against the pinned amplify-swift 2.58.1 SPM checkout source. Both BUILD SUCCEEDED, committed to main (`8d27c3b`, `08c1d1e`).
  - [[tyler/meta/session-2026-07-01-cc3305-3306-crm-author-fix-qp-notes-backfill]] — 2026-07-01: cc-3305 crm-write-api author fix — `sso-authorizer-v1` puts a JSON-encoded string under `authorizer.user` and a hardcoded `cognitoSub` constant (never attribute from it); `_principal` now `json.loads(user)` → email/user_id, verified non-NULL `handled_by_agent_id` live + graceful NULL for unknown emails. cc-3306 `backfill_qp_notes` mode: gg.message export → crm_activities (`qpmsg-` dedup + ON CONFLICT DO NOTHING, source=qp, is_internal=true, phone→contact, unmatched parked unattached); stage export generated from the gg DB (customer_contact empty — phones via hubspot_deals_data), 192 loaded (24 attached/168 parked), re-run proven no-op. `deploy.sh` re-puts the role policy every deploy — IAM grants must live in the script.
  - [[tyler/meta/session-2026-07-01-cc18-26-comms-admin-dialpad-backfill]] — 2026-07-01: comms-admin frontend UX (cc-18 mfa_required/Forbidden distinction, cc-19 feed/call detail, cc-21 gate broken recording playback, cc-22 self-serve TOTP enrollment) + gunnerteam-api root-cause fix + deploy (cc-23, PR #7's dialpad-enrich content-type/was_recorded/transcript-type validation, v428) + two-stage backfill (cc-24 read-only assessment: 134/136 transcripts polluted, 176 phantom recording keys 100% text/html & 0% recoverable; cc-25 executes the nulling+S3-delete mutation, exact-count-verified; cc-26 one-off script recovers 95/96 rows the cron's 24h window permanently misses, v429). Surfaced: Lambda Node 20.x silently bundles undeclared `@aws-sdk/client-cloudwatch`; `MIGRATION_SECRET` needs rotation.
  - [[tyler/meta/session-2026-07-01-cc2925-3304-masterdb-crm-write-api-migrate-pipeline]] — 2026-07-01: org_slug decoy fix part 2 (`org_id` on `/v1/auth/login`, AdminUi default, cc-2925/2926); new `crm-write-api` Lambda (`POST /crm/activity`, cc-3301, initially blocked on missing `is_internal` column); prod SST redeploy fixing the stale-bundle KeyError + real SST-4.15.2/aws-7.20.0 version bump (cc-3302); `masterdb_migrate` role provisioned via master exception (cc-3303); `w1` applied via migrate-prod pipeline (cc-3304) — surfaced and fixed two real infra bugs: OIDC `job_workflow_ref` trust never authorizes in this account (switched to `sub`), and `crm_*` tables were grant-only not owned by `crm_app` (new `x1_crm_ownership_to_crm_app` migration, k11 pattern). `crm-write-api` 201 verified end-to-end (worknote + comment).
  - [[gunnerteam/meta/session-2026-07-01-audit-freeze-guards-comms-admin-masterdb]] — 2026-07-01: **audit-write-timeout re-diagnosis** — cc-3200 enriched `[Audit] write failed` (action/path/ms, v419); first real line `ms=125745` vs 8s timeout + idle-cluster RDS metrics (ACU flat 25% floor, CPU 5–9%, 0 deadlocks, sub-ms writes) prove **container-freeze of fire-and-forget audit**, NOT capacity (Colin/Track B closed); continuous diurnal 317/24h, not a burst; cc-3201 flush must use AsyncLocalStorage (top offender `dialpad.sms.received` passes no `req`). Plus cc-1200 admin+manager invite subs (v425, `e02f307`), cc-1201 deny subs on receipt routes (v426, `db89ba9`), iOS cc-3105 weather danger badge (`024bc7f`, BUILD SUCCEEDED), comms-admin cc-10/11/12 (pin `aws@6.79.0` for BucketV2 #4471 → `sst deploy` unblocked, reconciled live==source, merged to main `4e5d5c6`), masterdb cc-2922 migrate role PR #22 + cc-2923 migrate pipeline draft PR #23. ⚠️ gunner-ios remote URL has an embedded PAT — rotate.
