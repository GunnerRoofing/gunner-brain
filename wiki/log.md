## [2026-07-02] save | Session — SMS bridge: Finder blocked, cc-3310 registry+endpoint, cc-3311 webhook deltas
- Type: session (masterdb + crm-transform; Finder-side task blocked)
- Location: wiki/tyler/meta/session-2026-07-02-cc3310-3311-sms-bridge-finder-blocked.md
- From: three-task Finder SMS-bridge arc. Task 1 (Finder receiver + pair registration) BLOCKED — repo unreachable (Doug's Lead Finder; not in ~/Dev, not in the 50 GunnerRoofing repos, org code search 0 hits for the router; only gunner-ios links out to finder.gunnerroofing.com); refused to scaffold a parallel stack. cc-3310: masterdb `y1_sms_bridge_subscriptions` (RLS-free pair registry, unique (org,send,receive), S/I/U→gunnerteam_app SELECT→crm_app) + service-authed idempotent `POST /v1/integrations/subscriptions` — applied to prod via migrate-prod workflow (w1→y1), smoke-tested (200/200-bump/422/401, verify_y1 rows {total:1,e164_clean:1,reregistered:1}). cc-3311: crm-transform `P4-finder-notify` third WATERMARKS source — HMAC-signed per-message deltas both directions, Python-side `_to_e164` pair match (census 1528/1528 clean), unconfigured→skip-with-held-cursor; live-fired vs throwaway HMAC receiver (3 delivered sig_valid, tamper 401, idempotent re-run, unroutable holds cursor), all debris deleted.
- ⏳ Handoffs: svc key `finder-sms-bridge` (/tmp file → Keeper → Doug); `crm-finder-webhook` secret → Doug; real path is `/v1/integrations/subscriptions` NOT `/api/v1/subscriptions`; Finder repo access or route to Doug; on Finder ship — set FINDER_WEBHOOK_URL + watermark pre-stamp decision.
- 🪤 Traps: migrate Lambda payload `{"action":"upgrade_to","target":…}` not `_alembic`; `_provision_service_key` broken as `masterdb_migrate` (no INSERT on service_clients + FORCE-RLS GUC); masterdb zips carry the tree TWICE (root + gunner_masterdb/) — patch both; CLI Function-URL AuthType=NONE 403s until `add-permission --invoked-via-function-url` (InvokeFunctionUrl statement alone insufficient).

## [2026-07-02] save | Session — cc-40→56 comms-admin light-suite console redesign (+ iOS cc-1203/1205 PII-cache purge)
- Type: session (multi-repo: gunner-comms-admin + gunner-masterdb + gunner-ios)
- Location: wiki/tyler/meta/session-2026-07-02-cc40-56-comms-admin-console-redesign.md
- From: one long conversation (2026-07-01 16:32 → 2026-07-02 12:01, commit-dated) that turned the comms-admin read-only Dialpad viewer into a full light-suite admin console. cc-45 (masterdb `b7d550f`, PR #27 merged `8f45f4a`) admin API trusts Cognito RS256 + `gt-admin`; cc-46 CRUD tabs (Orgs/Users/Contacts/Projects/Audit); cc-47 motion/a11y/contrast-guard/perf; cc-49 console → `admin.gunnerroofing.com` (Cloudflare stale-ACM gotcha, [[shared/cloudflare-sst-custom-domains]]); cc-51 full-width top bar + `fmtPhone`/`fmtWhen` + working All-reps pill; cc-52 thread labels the customer side (fixes "rep talking to himself"); cc-54 CSP `media-src` bucket-subdomain wildcard so recordings play (deployed); cc-55 XSS-safe SMS linkifier + MMS-image links + long-wrap; cc-56 frictionless TOTP (autofocus/numeric/auto-submit-at-6) both screens. Also gunner-ios cc-1203/1205 PII-cache purge (`65c9db0`/`a481c50`) — `clearAllUserCaches()` on logout + `gunner.lastAuthUserId` user-change guard.
- 🎯 Real theme bug (cc-50, `1ba26ca`): `/theme` was SELECTing flat cols (`primary_color`…) that don't exist → always defaulted; actual schema is `gt_org_theme(org_id UUID PK, config JSONB, updated_at, updated_by)`. Now reads `config` JSONB; gunner org has NO row → palette stays light (verified via gunnerteam-api `_sql` read-only probe).
...
- From: REVISED Phase B (Leo voided the blanket source='qp' purge — snapshot ids ~110k vs live prod ≤7,370 disjoint, no collision). Read-only probe first (resolved Leo's truncated pixel-check UUIDs to full: `e3da304e-…` comment + `ab6cdb15-…` worknote, on contact 692cf941, posted today 14:55–14:56 = his eyeball done). Throwaway runner as crm_app, triple-guarded `DELETE … WHERE org_id AND source='manual' AND id=ANY(4 ids) RETURNING id` → exactly 4 deleted (2 cc-3307 eyeball + 2 Leo pixel-check), qp_count_after=1, manual_count_after=2. Runner+scratch deleted immediately.
- Decision (asked Tyler → KEEP): `qpmsg-110603` (row 7fcd9b82) retained. Ticket's stale "0 remain" line contradicts the REVISED body which defers pre-Oct-2025 history to a Leo call; not gating the load either way.
- ⚠ 2 out-of-scope source='manual' rows left untouched (`18b1d50f` worknote + `d39fda47` comment on contact 2b842ad9, 07-01 19:44 = cc-3301 verification cruft) — flagged for separate cleanup, not cc-3307's 4 ids. Final prod: source='qp'=1, source='manual'=2, dialpad untouched.

## [2026-07-02] save | Session — cc-3309: backfill_qp_notes skips test projects + export-quality STOP
- Type: session
- Location: wiki/tyler/meta/session-2026-07-02-cc3309-backfill-skip-test-projects.md
- From: added `SKIP_PROJECT_IDS={92,96,94,113}` (payload-overridable via `skip_project_ids`) to crm-transform's `backfill_qp_notes` — skip-set rows counted `skipped_test_projects` and skipped before contact resolution (no insert, no park); project_id compared as str (JSONL int/CSV str). Commit `d69ccd1` on main. Dry-run on prod export: source_rows=243, skipped_test_projects=133, would_insert=110 — mechanically exact.
- ⚠ STOP (ticket's own guardrail): the dry-run DISPROVES the premise that the 110 kept rows are real customer notes ~100% resolving. Only 91/110 resolve (82.7%); of those 91, **72 are system "requested a GAF measurement" auto-notifications**, and the 5 resolvable phones are all test/placeholder contacts (`+…june05test…`, `+…qpjune5`, "Test Test", and `+15555555555` collapsing 54 rows from ~20 projects onto ONE contact = false attribution). Skip set itself is clean (all 133 have phone=None + offshore senders — no real note wrongly excluded). Real load NOT run — held for Leo: (1) is this the right export (reads as stage/QA, not prod)? (2) exclude system GAF notifications (sender/text rule, out of cc-3309 scope)? (3) park placeholder-phone rows instead of resolving? Consistent with cc-3307's blocked prod-export item.

## [2026-07-02] save | Session — cc-3308: crm-write-api CORS headers on all responses
- Type: session
- Location: wiki/tyler/meta/session-2026-07-02-cc3308-crm-write-api-cors.md
- From: UAT-blocking bug — API-GW AWS_PROXY means crm-write-api must emit its own Access-Control-* (the /crm/activity OPTIONS mock only covers preflight); real 201/400/404/500 responses carried none, browser blocked reads of successful writes → false failures + double-posts. Fix: `_resp(status, obj, event)` is the single choke point every return goes through (incl. the except→500 path); added allowlist-based `_cors_headers(event)` merged there once (ALLOWED_ORIGINS env, echoes matching Origin else falls back to first allowlisted — never wildcard). Also fixed deploy-write.sh: update-function-code branch never touched env vars (only create-function did) — new env vars would silently never land on redeploy; added update-function-configuration + jq-built JSON (AWS CLI shorthand breaks on commas inside a value).
- Verified live against crm-write-api (prod masterdb via gunnerteam-dev-masterdb-proxy, per cc-2111): 201 and 400 both carry the CORS header; untrusted Origin does NOT get echoed (allowlist enforced, not pass-through); forced a genuine 500 (NUL-byte param) — header present there too. Real test row deleted via the documented one-off-Lambda pattern (dry-run SELECT → DELETE by id → verify 0 rows → delete probe Lambda) after an MFA-session-expiry interruption mid-cleanup. Commit f5a99fb pushed to main.
- Open: Leo's manual qp-stage Sell UAT (the real acceptance bar) not exercised this session; origins (qp-stage/prod QP, sourced from the wiki env map + DNS, not confirmed live by Leo) need his sign-off.
...
- From: nine chained cc-prompts across gunner-comms-admin (frontend) and gunner-ios/gunnerteam-api (backend). cc-18 distinguishes `mfa_required` from a genuine `Forbidden` (both rendered the same "Access Denied" screen — the exact ambiguity that sent cc-17 chasing a phantom DB bug); cc-19 surfaces `internal_number`/`duration_ms`/`was_recorded`/`final_state` in the feed table and call detail (Rep/Customer/Gunner-line columns, From→To, m:ss durations); cc-21 confirms the reported broken recording is genuinely `text/html` in S3 (an ingestion bug, not a viewer bug) and gates the R badge/Play button on `was_recorded && has_recording` + honest error copy; cc-22 adds self-serve Amplify TOTP enrollment so a newly-granted admin isn't stuck at a dead end. Then the real fix: cc-23 deploys PR #7's `dialpad-enrich` validation (require `audio/*` content-type before upload, filter recording query to `was_recorded=true`, filter transcript lines to `type==='transcript'`) as gunnerteam-dev-api v428 — live-verified a real fresh call getting clean transcript + genuine `audio/mpeg` recording. cc-24 (read-only) quantifies existing damage via full census, not sampling: 134/136 transcripts polluted (133 recoverable), 176 phantom recording keys all `text/html` and 0% recoverable (Dialpad never recorded these calls) vs 0% corruption on the 136 genuine `was_recorded=true` recordings. cc-25 executes the mutation (null 134 transcripts + delete 176 S3 objects + null their keys, all exact-count pre/post verified) and discovers the enrich cron's `24h` date filter permanently strands 96 of the nulled rows — not a bug, a deliberate steady-state bound the backfill correctly declines to widen. cc-26 closes that gap with a one-off `backfill-transcripts.js` script (reuses the cron's own `dpFetch`/filter helpers, secret-gated `_task` invoke, v429) that recovers 95/96. Surfaced along the way: Lambda's Node 20.x runtime silently bundles `@aws-sdk/client-cloudwatch` even though it's undeclared in package.json (explains why the cron works in prod but fails `require()` locally); a client-side CLI read-timeout retry caused two overlapping Lambda invocations during cc-26 (harmless — the UPDATE is idempotent, zero duplicate rows resulted); `MIGRATION_SECRET` leaked in a CLI error echo during cc-24 and needs rotation.
...
- From: 10 task prompts across 4 repos. Headline: cc-3200 enriched `[Audit] write failed` (action/path/ms, gunnerteam-api v419) — first real post-deploy line `ms=125745` vs 8s `AUDIT_WRITE_TIMEOUT_MS` + idle-cluster RDS metrics (prod sczazkvf: ACU flat 25% floor never scaled, CPU 5–9%, DBConns 14/8, borrow-latency sub-ms, WriteLatency 0.3ms, 0 deadlocks) prove **container-freeze of fire-and-forget audit writes**, NOT shared-cluster capacity — refutes an interim "burst→capacity/Colin" read (artifact of sort-desc-limit-20) and vindicates cc-3201 freeze fix; failures are continuous diurnal (317/24h), not a burst; PI disabled so freeze inferred from ms≫timeout + distribution. cc-3201 design gap: top offender `dialpad.sms.received` is fire-and-forget with no `req` → flush queue must be AsyncLocalStorage-bound. gunnerteam-api cc-1200 (admin+manager invite subcontractors, gate split, v425 e02f307) + cc-1201 (deny subs 403 on /fieldportal receipt extract/commit — financial write path, v426 db89ba9); I/O-free auth-gate test suites; 189 units green. gunner-ios cc-3105 (severe-weather danger badge on job screen, themeManager tokens, weather.danger push routing, BUILD SUCCEEDED, 024bc7f). gunner-comms-admin: GitHub backup (private repo + both branches), cc-10 committed cc-06 fixes (05ccea4, 48/48), cc-11 pinned pulumi `aws@6.79.0` to fix BucketV2 alias bug #4471 → `sst deploy --stage dev` clean + reconciled live==source (AuthFailure401 metric emits, live IAM==committed), cc-12 merged cc-08-db-tls-verify-ca→main (4e5d5c6). gunner-masterdb cc-2922 (masterdb_migrate NOSUPERUSER NOBYPASSRLS role revision on u1_merge_weather_crm, PR #22 CI-green) + cc-2923 (in-VPC migrate Lambda + workflow-ref-scoped OIDC + Free-path migrate-prod.yml, draft PR #23, NOT deployed — Colin runs windowed prod deploy). ⚠️ SECURITY: gunner-ios `origin` remote URL has an embedded GitHub PAT (plaintext in .git/config) — rotate + switch to keychain/SSH. Live role-token checks (invite 200/403, sub 403, comms-admin mfa_required 403) left unverified by design (would need prod Cognito/DB mutation).
