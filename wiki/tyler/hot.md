...
# Tyler Hot Cache — 2026-06-30

## Current State
- **Lambda:** **v421 live** (`gunnerteam-dev-api`, alias `live`). Last session (2026-06-30): [[gunnerteam/meta/session-2026-06-30-cc2211-3201-whitelabel-audit-flush]] — cc-2211/2212 white-label copy de-hardcode (invite/reset emails, join push, shared email.js FROM/`<title>`, invite/reset deep-link pages, all via `resolveOrgName`; v418→v420); cc-3201 audit flush-before-freeze (AsyncLocalStorage per-request queue, bounded by `AUDIT_FLUSH_TIMEOUT_MS`, wraps `lambda.js` dispatch; SOC 2 CC7.2; v421). 22/22 unit tests. Commits de4965a/1e7ae8a/48fe55b.
...
- **🎨 White-label resolver:** `resolveOrgName(orgId)` (`lib/assistant-tasks.js`) = the shared name resolver — `gt_org_theme.config` brand/name → `organizations.name` → `'your company'`, per-container cached. `gt_org_theme.config` is colors-only today (8 hex keys enforced by `PATCH /org/theme`), so brand override is future-proofing; today it always falls back to `organizations.name` (= "Gunner Roofing" for org `69aad261`). `email.js buildFrom(name)` builds the RFC 5322 FROM with CR/LF/`"`/`<>` injection-stripping; internal alert/form emails keep the `'Gunner Team'` default.
