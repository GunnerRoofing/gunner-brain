...
## Current State
- **Lambda:** **v341 live** (`gunnerteam-dev-api`, alias `live`, RoutingConfig null). cc-2126 added an org-ownership preflight on the client-supplied `jobId` in `POST /time/checkin` + extracted a single shared FP client (`lib/fieldPortalClient.js`). cc-2125 gated the last unauth forms routes. cc-2124 moved `DB_PASSWORD` out → **env holds ZERO secrets** (config + `SECRETS_PATH` only); cc-2123 moved ~17 app secrets out. cc-2118 HS256→Cognito-RS256-only. On v330 base: cc-2106 multer/node-forge/node-apn, cc-2102 DB TLS.
- **iOS build:** BUILD SUCCEEDED — cc-1100–1110, cc-1200–1202, cc-1300, cc-2116 cert-pinning, cc-2117 jailbreak-detect on `main`. Tests: 50 outbox/offline + 9 JailbreakDetector = 59 in GunnerTeamTests.
- **Last session:** cc-2126 (2026-06-20) — **org-ownership preflight on client `jobId`** (commit `e607ae2`, deployed **v341**, CC6.1). `POST /time/checkin` took `req.body.jobId` and both wrote it to org-scoped `gt_time_entries` AND forwarded it to Field Portal with NO org check (CLAUDE.md rule violation). Added the proven `fieldportal.js` pattern: `ccFetch(GET /projects/:jobId)` before any write/proxy → throws on non-2xx (incl 404) → `.catch`→null→**404 job_not_found** (never leaks existence; ~5s ccFetch timeout). **Single FP client:** extracted `ccFetch`(+BASE/apiKey/upstreamTimeoutMs/upstreamFetch) from `fieldportal.js` into **`lib/fieldPortalClient.js`**; both `fieldportal.js` + `time.js` import it (no dup client). Checkout unaffected (server-side `rows[0].job_id`). forms.js/Monday IDs out of scope. Verified: /health 200, migration probe ok:true (refactored graph loads live), unauthed /time/checkin→401. ⚠️ **Tradeoff (prompt-accepted fail-closed):** check-in now needs FP reachable (FP down → 404, not success+fire-and-forget). Authed 200/404 verified by construction+parity (no Cognito token to forge). prior cc-2125 forms lockdown (v340)
