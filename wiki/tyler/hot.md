...
- **Last session:** cc-2135 — **A4 close-out diagnostic** (temp `[cc-2135]` log on `fieldportal.js` `/jobs` + `/jobs/bundle`, deployed **v347** → removed **v348**, **net-zero code**). Test account (Tyler signed in) opened Jobs → app showed "no jobs assigned". Logs settled it: **`gtRole=user`, `forwardedEmail=admin@gunnerroofing.com`** (the test account's **own** email — no `userEmail` param, so `resolveTargetEmail` returns `req.user.email`), **`projectsReturned=0`** on both paths. **Not a GunnerTeam bug:** we forward the user's own email at `user` role (no privilege escalation, no wrong identity); Colin correctly scopes that email to 0 (no FP project memberships → empty by design). The earlier "seeing all jobs" symptom is **not reproducible** — it was stale client cache (JobPreloadStore) before Colin's scoping went live / before a fresh fetch; current empty state is correct. ⚠️ If `admin@gunnerroofing.com` is *expected* to have assignments, that's a Colin-side FP-membership question, not ours.
