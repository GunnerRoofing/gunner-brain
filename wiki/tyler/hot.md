...
## Current State
- **Lambda:** **v371 live** (`gunnerteam-dev-api`, alias `live`, RoutingConfig null). `LLM_PROVIDER=anthropic` (temporary bridge — Bedrock billing-blocked). `/assistant/run` draft→200 (`claude-haiku-4-5-20251001`), extract→403. DB_USER=postgres (B1 forward flip still pending p17). See [[tyler/meta/session-2026-06-25-bedrock-billing-qp-key-org-reconcile]].
- **🌉 LLM provider = anthropic (bridge, v371):** flipped because Bedrock is blocked on `INVALID_PAYMENT_INSTRUMENT` for acct `980921733684` (Eddie/payer `661095510147` must add a valid card; model agreements ALREADY accepted). **Revert when card resolves:** `ssm put LLM_PROVIDER=bedrock` → `tf apply -target` → publish → alias. One-line, no other changes. In-account Bedrock is still the end state.
